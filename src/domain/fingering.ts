...
/** Total semitone drop of a button state. */
export function maskDrop(mask: number): number {
  return maskToValves(mask).reduce((drop, valve) => drop + VALVE_SEMITONES[valve], 0);
}

/**
 * What a button state actually sounds, for a player aiming at a given note.
 *
 * A valve combination does not pick out a pitch — it picks out a whole column of
 * the harmonic series, every note of which shares that fingering. Which one
 * comes out is decided by the lips, and the app has no way of knowing. So it
 * assumes the player is aiming at the note in front of them and takes whichever
 * member of that column lies nearest: hold valve 1 where F sharp was wanted and
 * you get F; hold it where B natural was wanted and you get B flat.
 *
 * Holding the correct fingering always returns the target exactly, since the
 * target is by definition a member of its own column.
 *
 * The virtual 4th valve is added only when the *target* needs one. Offering it
 * unconditionally would give every button state a second column to choose from,
 * and wrong fingerings would keep landing on the right note by accident — which
 * would defeat the entire point of sounding what the player fingered.
 */
export function soundedPitch(
  mask: number,
  targetSounding: number,
  instrument: Instrument,
): number {
  const primary = primaryFingering(targetSounding, instrument);
  const drop = maskDrop(mask) + (primary?.usesFourth ? VALVE_SEMITONES[4] : 0);

  let best = instrument.fundamentalMidi - drop;
  let bestDistance = Infinity;

  for (const partial of USABLE_PARTIALS) {
    const pitch = instrument.fundamentalMidi + partialSemitones(partial) - drop;
    const distance = Math.abs(pitch - targetSounding);
    // Ties go to the lower note, which is the way a missed fingering usually falls.
